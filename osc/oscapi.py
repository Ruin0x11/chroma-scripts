...
class ColorsIn:
    def set_colors(self,unused_addr, args):
        try:
            chroma = ChromaMessage.fromOSC(args)
    ...
    def whatever(self, unused_addr, args):
        print("dood")
    ...
    def start(self):
        ...
        self.dispatcher.map("/setcolors", self.set_colors)
        self.dispatcher.map("/dood", self.whatever)
        self.dispatcher.map("/reloadconfig", self.reload_config)
        self.server = osc_server.ThreadingOSCUDPServer(("localhost", 11661), self.dispatcher)
...
class ChromaMessage:
    ...
    def toOSC(self, addr):
        ...
        msg.add_arg(8)
        msg.add_arg(os.getpid())
        msg.add_arg(self.title)
        msg.add_arg(self.streamclass)
        msg.add_arg(self.framenumber)
        msg.add_arg(time.time())
        msg.add_arg(self.creator)
        msg.add_arg(self.description)
        for i in range(0, MAX_LIGHTS):
            msg.add_arg(self.data[i][0])
            msg.add_arg(self.data[i][1])
            msg.add_arg(self.data[i][2])
        return msg.build()
...
class ColorsOut:
    def __init__(self, streamclass="something"):
        self.client = udp_client.UDPClient("localhost", 11661)
        ...
        try:
          ...
          with open(path+"manifest.json") as data_file:
